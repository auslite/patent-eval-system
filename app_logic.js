...
const BmobAPI = {
    ...
    async get(table, where = {}, order = '-createdAt', limit = 200) {
        let url = `https://open2.bmob.cn/1/classes/${table}?limit=${limit}&order=${order}`;
    ...
    async post(table, data) {
        const res = await fetch(`https://open2.bmob.cn/1/classes/${table}`, {
            method: 'POST',
    ...
    async put(table, objectId, data) {
        const res = await fetch(`https://open2.bmob.cn/1/classes/${table}/${objectId}`, {
            method: 'PUT',
            headers: this.headers,
            body: JSON.stringify(data)
        });
...
function isCloudMode() { return bmobReady; }
